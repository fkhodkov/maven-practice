...
public final class ClassCounter extends AbstractMojo {
  /**
   ...
   * violated and throwIfFailed parameter is set to true.
   *
   * @throws MojoExecutionException If number of classes is less than
   *                                minClassNumber and throwIfFailed set to
   *                                true.
   */
  ...
  private void countClasses(final Path dir) throws IOException {
  ...
  private void reportNotEnoughClasses() throws MojoExecutionException {
    String msg =
        String.format("Not enought classes: only %d top-level classes found while %d is required",
